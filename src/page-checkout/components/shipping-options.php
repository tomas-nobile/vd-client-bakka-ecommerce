...
function etheme_render_checkout_shipping_options() {
	...
	?>
	<section class="rounded-2xl border border-gray-200 bg-white p-6 md:p-7" aria-labelledby="checkout-shipping-options">
		<h2 id="checkout-shipping-options" class="text-xl font-bold text-gray-900">
			<?php esc_html_e( 'Shipping options', 'etheme' ); ?>
		</h2>
		<p class="mt-1 text-sm text-gray-500">
			<?php esc_html_e( 'Choose the delivery option that works best for you.', 'etheme' ); ?>
		</p>
		...
		<?php if ( empty( $shipping_rates ) ) : ?>
			<p class="mt-5 rounded-md border border-dashed border-gray-300 bg-gray-50 px-4 py-3 text-sm text-gray-600">
				<?php esc_html_e( 'Shipping methods will appear after entering a valid address.', 'etheme' ); ?>
			</p>
		<?php else : ?>
			<div class="mt-5 space-y-4">
				<?php foreach ( $shipping_rates as $package_index => $package_rates ) : ?>
					<?php foreach ( $package_rates as $rate ) : ?>
						...
						?>
						<label
							class="checkout-shipping-option block cursor-pointer rounded-xl border border-gray-200 p-4 transition hover:border-gray-900 <?php echo $checked ? 'is-selected border-gray-900 bg-gray-50' : ''; ?>"
							...
							<div class="flex items-center justify-between gap-3">
								<div class="flex items-center gap-3">
									...
									<div>
										...
										<p class="text-xs text-gray-500"><?php esc_html_e( 'Estimated delivery based on your address.', 'etheme' ); ?></p>
									</div>
								</div>
								<p class="text-sm font-semibold text-gray-900">
									<?php echo $rate->get_cost() > 0 ? wp_kses_post( wc_price( $rate->get_cost() ) ) : esc_html__( 'Free', 'etheme' ); ?>
								</p>
							</div>
						</label>
					<?php endforeach; ?>
		...
		<p class="sr-only" aria-live="polite"><?php esc_html_e( 'Shipping method selection updated', 'etheme' ); ?></p>
	</section>
	<?php
}
